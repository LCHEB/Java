...
public class ArrayMethodEx1 {
	...
	public static void main(String[] args) {
		...
		System.out.println();
		//1~45 사이의 중복되지 않은 수 6개를 입력하는 코드를 작성하세요.(메서드를 이용)
		int[] user = new int[6];
		int count = 0;
		Scanner scan = new Scanner(System.in);
		System.out.print("정수 6개 입력 : ");
		while(count < user.length) {
			int num = scan.nextInt();
			if(!contins(user, count, num) && (num >= min && num <= max)) {
				user[count] = num;
				count++;
			}
		}
	 ...
	 * 매개변수 : 배열, 최소값, 최대값 => int arr[], int min, int max
	...
	// [1, 2, 0, 0, 0] => 배열에서 0번지부터 2개 중에서 일치하는 값이 있나? false
	 ...
	 * 메서드명 : contains
	 */
	public static boolean contins(int []arr, int n, int num) {
		//배열 생성이 안됐거나 크기가 0이면
		if(arr == null || arr.length == 0) {
			return false;
		}
