...
관리자 기능
 ...
 - 요금 관리
    ...
    사용자 (영화,극장,날짜)
    회원 (상영시간(FK), 좌석, 인원, 예매 번호, 예매 좌석, 예매 인원 정보)
    관리자 (아이디(PK), 감독, 배우, 영화, 극장, 상영관, 상영시간, 요금)
    감독 (인물, 역활, 대표사진)
    배우 (인물, 역활, 대표사진)
    극장 (지역, 이름, 상영관 정보(FK), 주소, 총 관수)
    인물 (이름, 생년월일, 내용, 국가)
    영화 (영화번호(PK), 감독(FK), 배우(FK), 장르, 기본정보, 개봉일, 내용, 스틸컷, 트레일러)
    상영관 (상영관 번호(PK), 총 좌석수, 좌석 정보)_+
    상영시간 (상영관정보(FK), 영화(FK), 날짜, 상영시간(PK))
    요금 (성인, 조조, 청소년)
