...
- 조회(상영시간)
    ...
    - 사용자는 영화, 극장, 날짜를 선택하면 극장 정보(영화관 이름, 전체좌석수)와
	  상영시간 별 현재 예매 가능 좌석수를 조회할 수 있다
- 조회(영화 상세 조회)
	- 사용자는 영화 상세정보를 조회할 수 있다
    - 상세 정보는 감독, 배우, 장르, 기본정보, 개봉일, 트레일러, 스틸컷이 있다
    - 장르는 등록된 장르를 보여주며, 장르는 여러가지일 수 있다
    - 기본 정보에는 관람 나이정보, 런닝 시간(분), 제작국가가 있다
    - 제작 국가는 여러 국가가 될 수 있다
	- 감독은 여러 감독이 올 수 있다
    - 배우는 주연,조연 구분 없이 등록된 배우를 등록된 배우순으로 조회한다
- 검색
	- 사용자는 검색을 통해 영화를 조화할 수 있다
...
관리자 기능
 - 감독/배우 관리
	...
	- 감독/배우는 등록된 인물과 역활, 대표사진을 선택하여 등록한다
 ...
 - 영화 관리
    ...
    - 영화 정보는 영화에 참여한 감독, 배우, 장르, 기본정보, 개봉일,
	  내용, 스틸컷, 트레일러를 입력하여 등록한다
 ...
 - 극장 관리
    ...
    - 극장은 지역, 이름, 상영관 정보, 주소, 총 관수를 입력하고,
      총 관수, 총 좌석수는 등록된 상영관에 맞게 계산이 된다
	- 지역은 등록된 지역 중 하나를 선택한다
    - 상영관 정보는 극장 등록과 함꼐 등록하거나 극장 등록 후 등록할 수 있다
 - 상영관 관리
	- 관리자는 상영관을 등록/수정/삭제할 수 있다
	- 극장 등록 시 상영관을 추가하는 경우, 상영관 번호, 총 좌석수, 좌석 정보를 입력하여 둥록한다
 ...
 - 요금 관리
	...
	회원 (아이디(PK), 비번)
    극장 (극장번호(PK), 지역, 이름, 주소, 총 관수)
    영화 (영화번호(PK), 제목, 장르, 기본정보, 개봉일, 내용)
    스틸컷(스틸컷번호(PK), 장면, 영화(FK))
    트레일러(트레일러번호(PK), 영상, 영화(FK))
    상영관 (상영관번호(PK), 총 좌석수, 좌석 정보, 극장정보(FK))
    상영시간 (상영시간번호(PK), 상영관정보(FK), 영화(FK), 날짜)
    감독 (감독번호(PK), 인물(FK), 역활, 대표사진, 영화(FK))
    배우 (배우번호(PK), 인물(FK), 역활, 대표사진, 영화(FK))
    인물 (이름(PK), 생년월일, 내용, 국가)
    예매 (예매 번호(PK), 날짜, 상영관정보(FK), 상영시간정보(FK), 아이디(FK))
*/