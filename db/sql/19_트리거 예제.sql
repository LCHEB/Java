...
delimiter //
create trigger insert_seat after insert on seat for each row
begin
	declare _sc_num int;
    ...
    update screen set sc_seat = _sc_seat where _sc_num;
...
#  상영관이 추가되면 영화관의 전체 상영관수를 업데이트 하는 트리거
drop trigger if exists insert_screen;

delimiter //
create trigger insert_screen after insert on screen for each row
begin
	declare _th_num int;
